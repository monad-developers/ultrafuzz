...
When searching for optional configuration or harness signals, keep each probe
separate and simple. Use one `grep -rIlE` probe by itself, or use `rg --files`
by itself and inspect the returned file names. Do not combine optional probes
with `;`, `echo` separators, `2>/dev/null`, or multi-stage filters such as
`| grep ... | head ...`. Missing optional files are ordinary evidence; record
...
`poetry.lock`, `uv.lock`, `package.json`, and Hardhat task/config files.
