...
- If no complete entrypoint trace is established, but the cited source shows
...
- If direct public reachability is unclear, classify it as `undetermined` and
  add `reachability=public-wrapper-required` plus the exact public entrypoint
  evidence needed before severity can treat it as production exploitable.
...
```json
{
  ...
  "notes": [
    ...
    "helper_proof=direct helper mismatch identified",
