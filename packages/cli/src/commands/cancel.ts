...
export default class Cancel extends Command {
  static override summary = "Cancel a linked run; cancellation is terminal";
  ...
  async run(): Promise<void> {
    ...
    emitCommandResult(
      this,
      "cancel",
      commandFromRuntime("cancel", result, (value) =>
        value.confirmed
          ? `Cancellation confirmed: ${value.run_id} is ${value.run_status}\n`
          : `Cancellation requested: ${value.run_id} is stopping\n`
      ),
      flags.json === true
    );
  }
