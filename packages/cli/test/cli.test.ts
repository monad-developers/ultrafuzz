...
function tempProject(): string {
  const project = fs.mkdtempSync(path.join(os.tmpdir(), "ufz-cli-"));
...
function fakeSmithersEnv(project: string): Record<string, string | undefined> {
  installPinnedFakeRunner(
    ...
    [
      ...
      '    replay_label=""',
      '    previous=""',
      '    for argument in "$@"; do [ "$previous" != "--frame" ] || replay_frame="$argument"; [ "$previous" != "--label" ] || replay_label="$argument"; previous="$argument"; done',
      '    case "$replay_frame" in ""|*[!0-9]*) printf \'replay requires integer --frame\\n\' >&2; exit 64 ;; esac',
      '    printf \'%s\\n\' "$replay_label" > "${SMITHERS_FAKE_LOG}.replay-label"',
      "    printf '%s\\n' '{\"forkedRunId\":\"ultrafuzz-cli-run-replayed\"}'",
      "    ;;",
      "  fork)",
      '    fork_label=""',
      '    previous=""',
      '    for argument in "$@"; do [ "$previous" != "--label" ] || fork_label="$argument"; previous="$argument"; done',
      '    printf \'%s\\n\' "$fork_label" > "${SMITHERS_FAKE_LOG}.fork-label"',
      "    printf '%s\\n' '{\"forkedRunId\":\"ultrafuzz-cli-run-forked\"}'",
      "    ;;",
      "  timeline)",
      '    if [ "$2" = "ultrafuzz-cli-run" ] && [ -f "${SMITHERS_FAKE_LOG}.replay-label" ]; then',
      '      replay_label="$(sed -n \'1p\' "${SMITHERS_FAKE_LOG}.replay-label")"',
      '      printf \'{"data":{"timeline":{"runId":"%s","frames":[{"frameNo":7,"forks":[{"runId":"ultrafuzz-cli-run-replayed","branchLabel":"%s"}]},{"frameNo":44,"forks":[]}],"children":[{"runId":"ultrafuzz-cli-run-replayed","branch":"%s","frames":[],"children":[]}]}}}\\n\' "$2" "$replay_label" "$replay_label"',
      '    elif [ "$2" = "ultrafuzz-cli-run-replayed" ] && [ -f "${SMITHERS_FAKE_LOG}.fork-label" ]; then',
      '      fork_label="$(sed -n \'1p\' "${SMITHERS_FAKE_LOG}.fork-label")"',
      '      printf \'{"data":{"timeline":{"runId":"%s","frames":[{"frameNo":7,"forks":[]},{"frameNo":44,"forks":[{"runId":"ultrafuzz-cli-run-forked","branchLabel":"%s"}]}],"children":[{"runId":"ultrafuzz-cli-run-forked","branch":"%s","frames":[],"children":[]}]}}}\\n\' "$2" "$fork_label" "$fork_label"',
      "    else",
      '      printf \'{"data":{"timeline":{"runId":"%s","frames":[{"frameNo":7,"forks":[]},{"frameNo":44,"forks":[]}],"children":[]}}}\\n\' "$2"',
      "    fi",
      "    ;;",
      "  pause)",
