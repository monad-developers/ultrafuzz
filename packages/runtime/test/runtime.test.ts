...
test("syncRun keeps incomplete zero-token events unpriced in segment and cumulative counts", async () => {
  const project = tempProject();
  initProject({ projectRoot: project, force: true });
  writeSmallTopology(project);

  const workflowRunId = "ultrafuzz-incomplete-zero-token-accounting";
  const env = fakeLifecycleSmithersEnv(project, {
    inspect: workflowInspect({
      workflowRunId,
      steps: [{ id: "node:project-discovery", state: "finished", attempt: 1 }]
    ...
    events: workflowEvents(workflowRunId, [
      { type: "NodeStarted", nodeId: "node:project-discovery", attempt: 1, extra: { iteration: 0 } },
      {
        ...
        extra: {
          iteration: 0,
          checkpointGenerationId: "generation-incomplete-zero",
          inputTokens: 0,
          outputTokens: 0
        }
      },
  ...
  const run = await startRun({ projectRoot: project, runId: "incomplete-zero-token-accounting", env });
  assert.equal(run.ok, true, JSON.stringify(run.diagnostics));
  writeRequiredArtifactSet(run.value!.run_root, "project-discovery", ["setup/project-discovery.md", "findings.json"]);

  const sync = await syncRun({ projectRoot: project, runId: "incomplete-zero-token-accounting", env });
  ...
  assert.equal(metadata.accounting?.cumulative?.event_count, 1);
  assert.equal(metadata.accounting?.cumulative?.priced_event_count, 0);
  assert.equal(metadata.accounting?.cumulative?.unpriced_event_count, 1);
  assert.equal(metadata.accounting?.cumulative?.usage_complete, false);
  assert.deepEqual(
    metadata.accounting?.cumulative?.usage_incomplete_reasons?.map((reason) => reason.code),
    ["attempt-identity-missing"]
  );
  assert.equal(metadata.accounting?.cumulative?.pricing_complete, false);
  assert.deepEqual(
    metadata.accounting?.cumulative?.pricing_incomplete_reasons?.map((reason) => reason.code),
    ["price-unavailable"]
  );
});
