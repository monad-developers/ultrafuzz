...
function createVerifiedReportFixture(
    ...
    nodes: [
      {
        ...
        workflow: {
          node_id: `node:${plannedAttemptIds[0]!}`,
          task_node_ids: plannedAttemptIds.map((plannedAttemptId) => `node:${plannedAttemptId}`)
        }
      }
    ]
  };
  ...
  const tasks = plannedAttemptIds.map((plannedAttemptId) =>
    smithersTaskForNode(layout, graph, graph.nodes[0]!, plannedAttemptId)
  );
  writeSealedTaskAuthority(layout, graph, tasks);
