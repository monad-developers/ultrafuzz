...
test("generated Smithers retries reset exact task-owned artifact contents after the first attempt", () => {
  ...
  assert.match(reset, /restoreWorkspacePatchPreparation\(task, workspaceRoot\)/u);
  assert.match(reset, /prepareTaskWorkspaceOutputRoots\(task, \{ replayWorkspacePatches: false \}\)/u);
...
test("generated Smithers agent preserves its final response as missing non-report Markdown", () => {
  ...
  assert.match(agent, /await postflight\("workspace-patch-materialization-postflight"/u);
  ...
  assert.match(agent, /prepareTaskWorkspaceOutputRoots\(task, \{ replayWorkspacePatches: false \}\)/u);
  ...
  assert.match(agent, /materializeWorkspacePatch\(task\)/u);
...
test("generated Smithers agent mirrors declared workspace tests before strict verification", () => {
  ...
  assert.match(materializer, /testRoot, "foundry", logicalNodeId, workspaceRelativePath/u);
  assert.match(materializer, /existingCandidates = sourceCandidates\.filter/u);
...
test("generated Smithers retry snapshots are durable and restore through canonical parents", () => {
  ...
  assert.ok(
    preparationRestore.indexOf("removeStaleWorkspaceFiles(workspaceRoot, preparationTree)") <
      preparationRestore.indexOf('["read-tree", "--reset", task.baseCommit]'),
    preparationRestore
  );
...
test("generated Smithers verifier publishes the complete validated set before task success", () => {
  const source = fs.readFileSync(workflowTemplatePath, "utf8");
  const verifierStart = source.indexOf("function collectVerifiedArtifacts");
  const workflowStart = source.indexOf("export default smithers");
  ...
  assert.ok(
    verifier.indexOf("publishVerifiedArtifacts(artifactDir, publications)") <
      verifier.indexOf("const primaryArtifact = primary.path")
  );
  assert.match(verifier, /artifact_set_digest: sha256Canonical\(\{ artifacts, primary_artifact: primaryArtifact \}\)/u);
  assert.match(verifier, /const verified = collectVerifiedArtifacts\(task\)/u);
});
...
test("generated Smithers preparation requires a successful dependency artifact verification", () => {
  ...
  const preparationStart = source.indexOf("function prepareArtifactMirror");
  const materializeStart = source.indexOf("function materializeInvariantSuiteFromDependencies");
  const collectorStart = source.indexOf("function collectVerifiedArtifacts");
  const verifierStart = source.indexOf("function verifyArtifacts");
  const workflowStart = source.indexOf("export default smithers");

  assert.ok(preparationStart >= 0, source);
  assert.ok(materializeStart > preparationStart, source);
  assert.ok(collectorStart > materializeStart, source);
  assert.ok(verifierStart > collectorStart, source);
  assert.ok(workflowStart > verifierStart, source);

  const collector = source.slice(collectorStart, verifierStart);
  const verifier = source.slice(verifierStart, workflowStart);
  ...
  assert.match(collector, /clearArtifactVerificationMarker\(task\)/u);
  assert.match(verifier, /writeArtifactVerificationMarker\(task, verified\.artifacts, verified\.publications\)/u);
  ...
  assert.notEqual(collector.indexOf("clearArtifactVerificationMarker(task)"), -1, collector);
  assert.ok(
    collector.indexOf("clearArtifactVerificationMarker(task)") < collector.indexOf("const artifactRoots"),
    collector
  );
  assert.ok(
    verifier.indexOf("writeArtifactVerificationMarker(task, verified.artifacts, verified.publications)") >
      verifier.indexOf("const output = verificationOutput.parse"),
    verifier
  );
});
...
test("generated Smithers preserves setup-patch baselines across post-agent preparation", () => {
  ...
  assert.match(
    source,
    /const result = await runAgentWithPostflight\([\s\S]*?\(\) => agent\.generate\(args\)[\s\S]*?prepareTaskWorkspaceOutputRoots\(task, \{ replayWorkspacePatches: false \}\)/u
  );
  assert.match(source, /assertAgentWorkspaceTreeProvenance\([\s\S]*?workspacePatchPreparationTree\(task\)/u);
  assert.match(
    source,
    /workspace-patch-materialization-postflight[\s\S]*?materializeWorkspacePatch\(task\)[\s\S]*?workspace-provenance-postflight[\s\S]*?assertAgentWorkspaceTreeProvenance\([\s\S]*?resultTree/u
  );
});
