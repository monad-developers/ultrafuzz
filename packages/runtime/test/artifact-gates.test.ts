...
function fixtureDeclaredContract(artifactPath: string): PlannedGraphNode["outputs"][number]["contract"] | undefined {
  ...
  if (artifactPath === "coverage-goal.json") return "ultrafuzz/coverage-goal@1";
...
test("coverage gate binds selected and unselected ranges to the trusted production inventory", () => {
  ...
  const publish = (value: unknown, markdown = scopedMarkdown, goalValue: unknown = goal): void => {
    writeArtifact(layout, node.id, "coverage-goal.json", JSON.stringify(goalValue));
...
test("coverage gate authenticates Vyper declaration boundaries outside the Solidity-only Recon selection", () => {
  ...
  const goal = {
    schema_version: "ultrafuzz.coverage-goal.v1",
    target: { scope: "selected-range", minimum_percent: 90 },
    current_measurement: { scope: "selected-range", covered_ranges: 0, total_ranges: 0 },
    current_status: "measured",
    planned_commands: [],
    stop_conditions: ["reserve time for finalization"],
    timeout_seconds: 60,
...
test("final report preserves finalized scoped coverage evidence and rejects bare percentages", () => {
  ...
  for (const mixedScore of [
    ...
    '<span class="metric">Coverage was 100%.</span>',
    '<p id="coverage-summary">Coverage was 100%.</p>',
    '<strong style="font-weight:bold">Coverage was 100%.</strong>',
    '<img class="chart" alt="Coverage was 100%.">',
    'Coverage was <span class="metric">100%</span>.',
    'Coverage was 1<span class="metric">00</span>%.',
    "Coverage was 10\uFE0F0%.",
    "Coverage was 99,5%.",
    ...
    assert.ok(
      mixed.diagnostics.some(
        (diagnostic) =>
          diagnostic.code ===
          (/%|٪|&(?:percnt|#0*37|#x0*25);|\b(?:pct\.?|per[ -]?cent(?:age)?)\b/iu.test(normalizedMixedScore)
  ...
  for (const exactScopedScore of [
    ...
    "Coverage was 100% — selected-range.",
    "Coverage was 100% in the selected-range scope.",
    "The selected-range view has coverage of 100%.",
    "Coverage was 100% under selected-range.",
    "Coverage was 100% (scope: selected-range).",
    "Selected-range is the scope; coverage was 100%.",
    "Coverage for the selected-range view was 100%.",
    "Coverage was 100% for both selected-range and production-source.",
    "Coverage was 100% across selected-range and production-source.",
  ...
  writeArtifact(layout, reportNode.id, "report.md", scopedMarkdown);
  const percentageIssue = {
    ...currentFinding("M-01", {
      title: "[M-01] - Property failure",
      dedupe_key: "root-coverage-percentage",
      triage_classification: "true-positive",
      ...
      stages: [
        {
          ...
          finding_id: "finding-coverage-percentage"
        }
      ]
    }
  };
  const percentageReport = currentReport(layout.runId, {
    coverage_evidence: evidence,
    issues: [percentageIssue]
  });
