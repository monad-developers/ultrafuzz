...
// Descriptor-alias modules stay in the file namespace: Bun neither routes imports from
// plugin-namespace modules through plugin resolution (bare and relative specifiers fall back to
// the process working directory) nor accepts a rewritten path from `onResolve`, so a custom
// namespace cannot express "validate, then load normally". Directly imported aliases are instead
// realpath-validated against the physical snapshot at resolve time and then loaded natively, which
// keeps every transitive import anchored to the same held-descriptor tree (an alias-relative
// resolution that leaves the descriptor prefix lands on a real path for the lexical filter to
// reject, and never consults the working directory). Alias-shaped paths are excluded from the
// lexical outside filter because the descriptor rules above, not lexical prefixes, govern them.
export const BUN_MODULE_CONFINEMENT_SOURCE = `import fs from "node:fs"; import path from "node:path"; import { fileURLToPath } from "node:url"; import { plugin } from "bun"; const sourceRoot = path.dirname(path.dirname(fileURLToPath(import.meta.url))), physicalRoot = fs.realpathSync(sourceRoot), descriptor = fs.openSync(sourceRoot, "r"), descriptorRoot = "/proc/" + process.pid + "/fd/" + descriptor, escape = (value) => [...value].map((character) => "^$.*+?()[]{}|\\\\".includes(character) ? "\\\\" + character : character).join(""), allowed = [sourceRoot, physicalRoot].map(escape).join("|"), aliasSource = "/proc/(?:(?:" + process.pid + ")|self)/fd/[0-9]+(?:/|$)", descriptorAlias = new RegExp("^" + aliasSource), outside = new RegExp("^(?!(?:" + allowed + ")(?:/|$))(?!" + aliasSource + ").+"), current = () => { if (fs.realpathSync(sourceRoot) !== physicalRoot || fs.realpathSync(descriptorRoot) !== physicalRoot) throw new Error("workflow controller snapshot changed during sealed resolution"); }, rejected = (value) => new Error("workflow controller module resolved outside its sealed snapshot: " + value + " (sealed snapshot: " + physicalRoot + ")"), resolveSealed = (value) => { current(); let resolved; try { resolved = fs.realpathSync(value); } catch { throw rejected(value); } if (resolved !== physicalRoot && !resolved.startsWith(physicalRoot + path.sep)) throw rejected(value); return resolved; }; plugin({ name: "ultrafuzz-sealed-modules", setup(build) { build.onResolve({ filter: descriptorAlias, namespace: "file" }, (args) => { resolveSealed(args.path); return undefined; }); build.onLoad({ filter: outside, namespace: "file" }, (args) => { current(); throw rejected(args.path); }); } });\n`;
...
export function verifyWorkflowControlSnapshot(
  ...
  const executionFiles = seal.execution_files.map((entry) => {
    ...
    return { sourcePath: entry.source_path, snapshotPath: entry.snapshot_path, contents: bytes, verifiedPath };
  });
  const baseGraph = executionFiles.find((file) => file.snapshotPath === DYNAMIC_BASE_GRAPH_SNAPSHOT_PATH);
  const baseTasks = executionFiles.find((file) => file.snapshotPath === DYNAMIC_BASE_TASKS_SNAPSHOT_PATH);
  ...
  const contents = controlFileContents(projectRoot, layout, paths, {
    ...
    ...(baseTasks === undefined ? {} : { tasks: baseTasks.contents }),
    ...(sealedWorkflowContents === undefined ? {} : { workflow: sealedWorkflowContents })
  });
...
function controlFileContents(
  ...
  layout: RunLayout,
  paths: WorkflowControlPaths,
  sealedContents: Partial<Record<WorkflowControlFileKey, Buffer>> = {}
): Record<WorkflowControlFileKey, Buffer> {
  return {
    graph: sealedContents.graph ?? readBoundedRegularFile(layout.root, paths.graphPath, "run graph"),
    expanded_graph:
      sealedContents.expanded_graph ??
      readBoundedRegularFile(layout.root, paths.expandedGraphPath, "expanded workflow graph"),
    graph_fingerprint:
      sealedContents.graph_fingerprint ??
      readBoundedRegularFile(layout.root, paths.graphFingerprintPath, "run graph fingerprint"),
    config: sealedContents.config ?? readBoundedRegularFile(layout.root, paths.configPath, "resolved workflow config"),
    tasks: sealedContents.tasks ?? readBoundedRegularFile(layout.root, paths.tasksPath, "workflow task manifest"),
    input: sealedContents.input ?? readBoundedRegularFile(layout.root, paths.inputPath, "workflow input"),
    workflow: sealedContents.workflow ?? readBoundedRegularFile(projectRoot, paths.workflowPath, "generated workflow"),
    evidence_workflow:
      sealedContents.evidence_workflow ??
      readBoundedRegularFile(layout.root, paths.evidenceWorkflowPath, "evidence workflow")
  };
}
