...
export const BUN_MODULE_CONFINEMENT_SOURCE = `import fs from "node:fs"; import path from "node:path"; import { fileURLToPath } from "node:url"; import { plugin } from "bun"; const sourceRoot = path.dirname(path.dirname(fileURLToPath(import.meta.url))), physicalRoot = fs.realpathSync(sourceRoot), descriptor = fs.openSync(sourceRoot, "r"), descriptorRoot = "/proc/" + process.pid + "/fd/" + descriptor, escape = (value) => [...value].map((character) => "^$.*+?()[]{}|\\\\".includes(character) ? "\\\\" + character : character).join(""), allowed = [sourceRoot, physicalRoot].map(escape).join("|"), outside = new RegExp("^(?!(?:" + allowed + ")(?:/|$)).+"), descriptorAlias = new RegExp("^/proc/(?:(?:" + process.pid + ")|self)/fd/[0-9]+(?:/|$)"), sealedNamespace = "ultrafuzz-sealed-module", loader = (value) => ({ ".jsx": "jsx", ".ts": "ts", ".mts": "ts", ".cts": "ts", ".tsx": "tsx", ".json": "json", ".toml": "toml" })[path.extname(value).toLowerCase()] || "js", current = () => { if (fs.realpathSync(sourceRoot) !== physicalRoot || fs.realpathSync(descriptorRoot) !== physicalRoot) throw new Error("workflow controller snapshot changed during sealed resolution"); }, rejected = (value) => new Error("workflow controller module resolved outside its sealed snapshot: " + value + " (sealed snapshot: " + physicalRoot + ")"), resolveSealed = (value) => { current(); let resolved; try { resolved = fs.realpathSync(value); } catch { throw rejected(value); } if (resolved !== physicalRoot && !resolved.startsWith(physicalRoot + path.sep)) throw rejected(value); return resolved; }; plugin({ name: "ultrafuzz-sealed-modules", setup(build) { build.onResolve({ filter: descriptorAlias, namespace: "file" }, (args) => ({ path: resolveSealed(args.path), namespace: sealedNamespace })); build.onLoad({ filter: /.*/, namespace: sealedNamespace }, (args) => { const resolved = resolveSealed(args.path); return { contents: fs.readFileSync(resolved), loader: loader(resolved), resolveDir: path.dirname(resolved) }; }); build.onLoad({ filter: outside, namespace: "file" }, (args) => { current(); throw rejected(args.path); }); } });\n`;
