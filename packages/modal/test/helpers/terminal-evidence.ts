...
export function writeTerminalEvidenceFixture(input: {
  ...
  const stateNodeIds = Object.keys(input.state.nodes).sort();
  ...
  const graph = {
    ...
    nodes: stateNodeIds.map((id) => {
      ...
      return { id, ...(taskNodeIds.length === 0 ? {} : { workflow: { task_node_ids: taskNodeIds } }) };
    })
  };
  const state = {
    ...input.state,
  ...
  const control = {
    ...
    bindings: {
      ...
      graph_fingerprint: graphFingerprint,
      config_fingerprint: configFingerprint,
      expected_state_node_ids: stateNodeIds,
