...
// This file's artifact-gate fixture builds a durable state by hand and names no control file, so it
// classifies through the shared adapter that supplies the bindings the real classifier requires.
// Calling the classifier directly with those two arguments alone answers `operational-failure` for
// every input, which would make the gated expectation below pass for the wrong reason.
import { classifyTerminalDispositionFromMinimalEvidence } from "./helpers/terminal-disposition-evidence.js";
...
describe("terminal artifact-gate recovery", () => {
  ...
  it("drives a real artifact-contract gate failure to the bounded no-progress recovery terminal", () => {
    ...
    expect(classifyTerminalDispositionFromMinimalEvidence(gated, TASK_MANIFEST)).toEqual({
    ...
    expect(classifyTerminalDispositionFromMinimalEvidence(repaired, TASK_MANIFEST)).toEqual({
      kind: "clean",
      failedTasks: 0,
