...
async function stageModalNodeResult(
  ...
  try {
    ...
    // Decided here, while staging, because the answer depends on the artifacts this machine
    // currently holds; `commit` replaces the artifact directory from the remote result before it
    // writes the marker, so by then they are the remote ones again.
    let markerRefreshed = false;
    if (canonical.verificationMarker !== undefined && verificationRoot !== undefined) {
      const verificationDestination = path.join(verificationRoot, path.basename(canonical.verificationMarker));
      markerRefreshed = isRefreshedVerificationMarker(
        canonical.verificationMarker,
        verificationDestination,
        artifactDir
      );
      assertPublishedFileReplacementAllowed(canonical.verificationMarker, verificationDestination, markerRefreshed);
    }
    let committed = false;
    return {
      commit: () => {
