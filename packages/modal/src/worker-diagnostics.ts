import { redactSecretValueRepresentations, redactSecretsInText } from "@ultrafuzz/security";
 ...
 *
 * The caller-supplied pass goes through `redactSecretValueRepresentations` rather than a literal
 * substring replacement, because a child process reports a credential in whatever encoding the layer
 * that failed happened to use -- JSON-escaped, base64, hex, percent-encoded. A literal compare only
 * catches the raw spelling and lets every other one through into a diagnostic this worker publishes.
 */
export function sanitizeWorkerDiagnosticMessage(
  ...
  let sanitized = redactSecretValueRepresentations(message, options.forbiddenSecretValues ?? []);
