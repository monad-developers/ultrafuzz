...
test("artifact schema snapshots are present and aligned with exported schema constants", () => {
  ...
  // The whole document must match, not only `$id`/`required`: a drifted property definition is
  // exactly how the snapshot and the runtime validator disagreed in both directions before.
  ...
  // A `$id`/`required`-only assertion let the run-state output-contract enum go stale, so the
  // published snapshot rejected every run that emitted a threat-model or goal-plan artifact.
  assert.deepEqual(runStateSnapshot, runStateJsonSchema);
  assert.deepEqual(readSchemaSnapshot("findings.schema.json"), findingsJsonSchema);
