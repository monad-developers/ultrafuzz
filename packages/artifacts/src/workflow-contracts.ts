...
export const INVARIANT_CAMPAIGN_PLAN_SCHEMA_VERSION = "ultrafuzz.invariant-campaign-plan.v1" as const;
...
export const WORKFLOW_CONTRACT_DESCRIPTIONS: Record<WorkflowContractId, string> = {
  ...
  "ultrafuzz/invariant-campaign-plan@1": "The exact invariant backend, CPU, deadline, reserve, paths, and commands.",
...
export const WORKFLOW_SEMANTIC_GATES: Record<WorkflowContractId, readonly string[]> = {
  "ultrafuzz/harness-repairs@1": ["unique-harness-repair-failure-ids"],
  "ultrafuzz/strategy-detections@1": ["unique-strategy-detection-hit-provenance"],
  "ultrafuzz/triaged-findings@1": ["preserve-upstream-finding-fields", "unique-finding-ids"],
  "ultrafuzz/severity-classified-findings@1": ["severity-matrix-equality", "preserve-triage-and-provenance"],
  "ultrafuzz/reference-manifest@1": ["reference-file-digest-and-existence"],
  "ultrafuzz/boundary-recipes@1": ["unique-boundary-recipe-ids"],
  "ultrafuzz/admin-config-boundary-matrix@1": ["unique-admin-surface-ids"],
  "ultrafuzz/dependency-scope-matrix@1": ["unique-dependency-ids", "in-scope-target-reference-integrity"],
  "ultrafuzz/externalized-state-accounting@1": ["state-component-reference-integrity"],
  "ultrafuzz/coverage-goal@1": ["coverage-status-measurement-consistency"],
  "ultrafuzz/invariant-campaign-plan@1": ["campaign-worker-cpu-equality", "campaign-deadline-budget-consistency"],
  "ultrafuzz/campaign-summary@2": ["campaign-count-and-reference-reconciliation"],
  "ultrafuzz/differential-plan@1": ["unique-differential-lane-ids", "lane-surface-reference-integrity"],
  "ultrafuzz/reference-harness@1": ["reference-model-surface-reference-integrity"],
  "ultrafuzz/audited-differential-lanes@1": ["ready-lane-attempt-assignment", "unique-ready-lane-ids"],
  "ultrafuzz/differential-lane-result@1": ["lane-result-source-payload-preservation"],
  "ultrafuzz/semantic-red-registry@1": ["unique-stable-failure-hashes"],
  "ultrafuzz/differential-red-triage@1": ["triage-pass-filename-agreement", "triage-repair-allowance"],
  "ultrafuzz/differential-repair-summary@1": ["cross-pass-unanimous-repair-authorization"],
  "ultrafuzz/differential-gap-review@1": ["audited-lane-result-coverage"],
  "ultrafuzz/differential-report-review@1": ["report-row-red-and-repair-reconciliation"],
  "ultrafuzz/dynamic-strategy-plan@1": ["selected-strategy-count-equality"],
  "ultrafuzz/dynamic-enumerator-outputs@1": ["unique-enumerator-ids", "unique-enumerator-recommendation-ids"],
  "ultrafuzz/selected-strategies@1": ["unique-selected-strategy-ids", "selected-plan-reference-integrity"],
  "ultrafuzz/dynamic-strategy-provenance@1": ["generated-file-strategy-reference-integrity"],
  "ultrafuzz/finding-lifecycle-ledger@1": ["unique-lifecycle-dedupe-keys", "lifecycle-stage-preservation"],
  "ultrafuzz/aggregation-manifest@1": ["aggregation-count-reconciliation", "aggregation-file-existence-and-byte-size"],
  "ultrafuzz/report@2": ["report-severity-matrix", "report-property-and-lifecycle-joins"]
};

export function workflowProjectedKeyGateIssues(contract: WorkflowContractId, value: unknown): string[] {
  if (contract !== "ultrafuzz/finding-lifecycle-ledger@1") return [];
  const parsed = findingLifecycleLedgerSchema.safeParse(value);
  if (!parsed.success) return [];
  const seen = new Set<string>();
  const duplicates = new Set<string>();
  for (const record of parsed.data.records) {
    if (seen.has(record.dedupe_key)) duplicates.add(record.dedupe_key);
    seen.add(record.dedupe_key);
  }
  return [...duplicates].sort().map((key) => `Duplicate lifecycle dedupe_key ${JSON.stringify(key)}`);
}
