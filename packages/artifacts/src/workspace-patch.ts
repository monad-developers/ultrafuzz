...
const workspacePatchPath = z
  .string()
  .min(1)
  .refine((value) => !value.includes("\u0000"), { message: "Workspace patch paths cannot contain NUL bytes" })
  .refine((value) => !value.includes("\\"), { message: "Workspace patch paths must use POSIX separators" })
  .refine((value) => !value.startsWith("/") && !/^[A-Za-z]:/u.test(value), {
    message: "Workspace patch paths must be target-relative"
  })
  .refine((value) => !value.split("/").some((segment) => segment === ".."), {
    message: "Workspace patch paths cannot traverse outside the target"
  })
