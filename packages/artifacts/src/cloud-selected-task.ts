...
/** The one field a deferred prompt rendering supplies after compilation. */
export const CLOUD_SELECTED_TASK_RUNTIME_PROMPT_FIELD = "promptPath";

/**
 * The only values that exist nowhere but in the expansion which generated an attempt.
 *
 * The expansion key names an item in a runtime document, and the two digests are hashes of runtime
 * bytes, so no compile-time constant can reproduce them. The schema still bounds all three, and the
 * generated attempt's label -- which composes the key -- is reconstructed from it rather than relaxed,
 * so every remaining field of a generated attempt is compared exactly.
 */
export const CLOUD_SELECTED_TASK_GENERATED_EXPANSION_FIELDS = [
  "metadata.node.dynamic.expansionKey",
  "metadata.node.dynamic.sourceDigest",
  "metadata.node.dynamic.itemDigest"
] as const;

/**
 * The compile-time derivation that ties a runtime dependency extension to a real materialization.
 *
 * Only the consumer knows which dynamic groups a compiled task declared and how each group derives
 * its generated attempt identities, so it supplies the derivation and the contract does the checking.
 */
...
export interface CloudSelectedTaskCanonicalOptions {
  /**
   * The compiled task declares dynamic dependencies, so runtime lowering extends its dependency
   * arrays -- but only with correlated evidence for one materialized generated attempt per entry.
   */
  runtimeDependencies?: CloudSelectedTaskRuntimeDependencyEvidence;
  ...
  /** The attempt itself was generated by expansion, so expansion-only provenance is runtime data. */
  allowsGeneratedExpansionValues?: boolean;
}

/**
 * Compares a handoff against the whole canonical compiled DTO.
 *
 * Every field is compared, including nested ones. The only relaxations are the documented runtime
 * fields above, and each relaxation still gets its own rule rather than being skipped: a runtime
 * prompt must be the attempt's own rendered prompt inside its own artifact directory, and a runtime
 * dependency extension must carry correlated evidence of the materialization that produced it.
 */
export function assertCloudSelectedTaskMatchesCanonical(
  ...
  if (options.allowsRuntimeRenderedPrompt === true) {
    ...
    relaxed.add(CLOUD_SELECTED_TASK_RUNTIME_PROMPT_FIELD);
  }
  if (options.runtimeDependencies !== undefined) {
    assertCorrelatedRuntimeDependencies(actual, canonical, options.runtimeDependencies);
    for (const field of CLOUD_SELECTED_TASK_RUNTIME_EXTENDED_FIELDS) relaxed.add(field);
  }
  if (options.allowsGeneratedExpansionValues === true) {
    for (const field of CLOUD_SELECTED_TASK_GENERATED_EXPANSION_FIELDS) relaxed.add(field);
  }
...
/**
 * Ties every runtime dependency extension back to the one materialization that produced it.
 *
 * Runtime lowering appends one materialized attempt at a time, and each append is a *set*: the
 * attempt ID, its artifact directory under the run root's `artifacts/`, and -- when the producer is
 * agentic -- its verifier node ID. Accepting the arrays as independent supersets would admit an
 * artifact directory with no attempt behind it, an attempt with no directory, or a verifier for an
 * attempt that was never added. Each entry is therefore correlated with one attempt ID, and every
 * extra attempt ID must in turn belong to a generated node one declared group could have produced.
 */
function assertCorrelatedRuntimeDependencies(
  actual: CloudSelectedTask,
  canonical: CloudSelectedTask,
  evidence: CloudSelectedTaskRuntimeDependencyEvidence
): void {
  const artifactDirs = runtimeExtension(
    actual.dependencyArtifactDirs,
    canonical.dependencyArtifactDirs,
    "dependencyArtifactDirs"
  );
  const attemptIds = runtimeExtension(
    actual.metadata.dependencies.attemptIds,
    canonical.metadata.dependencies.attemptIds,
    "metadata.dependencies.attemptIds"
  );
  const smithersNodeIds = runtimeExtension(
    actual.metadata.dependencies.smithersNodeIds,
...
/**
 * Returns the appended tail after checking the compiled entries survived unchanged, in order.
 *
 * Runtime lowering appends to the compiled array and de-duplicates the result, so the compiled
 * entries must remain its exact prefix: reordering or dropping one would silently redirect a
 * dependency, and a repeat would let one materialization be counted twice.
 */
function runtimeExtension(actual: readonly string[], canonical: readonly string[], field: string): readonly string[] {
  const keepsCompiledPrefix =
    actual.length >= canonical.length && canonical.every((entry, index) => actual[index] === entry);
  if (!keepsCompiledPrefix || new Set(actual).size !== actual.length) {
    throw new CloudSelectedTaskError(
      `selected_task ${field} must extend the compiled attempt without reordering, dropping, or repeating its entries`
    );
  }
  return actual.slice(canonical.length);
}
