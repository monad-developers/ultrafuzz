...
export type EvalExpansionReason =
  ...
  /** The ledger parsed, but no entry carries the `node_id` per-node cost is joined on. */
  | "usage-ledger-node-id-missing"
  /** The ledger carries join keys, but none of them names a node this lane actually ran. */
  | "usage-ledger-node-unmatched"
  /** The planner named this lane, but the run recorded no node for it, so there is nothing to join. */
