...
describe("prompt semantic anchors", () => {
  ...
  it("keeps the comparison prompt set free of generated-test collection contracts", () => {
    ...
    expect(promptCorpus).not.toContain("Solidity PoC code block");
  });

  it("keeps no-fuzz terminal reports compatible with the canonical issue contract", () => {
    const report = prompt("review/final-report.md");

    expect(report).toContain("canonical normalized finding");
    expect(report).toContain("`schema_version`, `id`, `title`, `status`,");
    expect(report).toContain("`severity_guess`, `confidence`, and `summary`");
    expect(report).toContain("canonical `strategy` field a non-empty originating strategy");
    expect(report).toContain("structured `strategy_provenance` object");
  });

  it("uses source-backed public reachability evidence without generated wrapper PoCs", () => {
    const promptCorpus = loadBuiltInPromptAssets()
      .filter((asset) => !asset.relativePath.startsWith("smoke/"))
      .map((asset) => asset.markdown)
      .join("\n");

    expect(promptCorpus).toContain("reachability=public-entrypoint-evidence-required");
    expect(promptCorpus).not.toContain("reachability=generated-public-wrapper-poc");
    expect(promptCorpus).not.toContain("reachability=public-wrapper-required");
    expect(promptCorpus).not.toContain("generated public wrapper PoC");
  });
