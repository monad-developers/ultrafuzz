...
// The NoFuzz control removes the stateful-invariant chain and every generated-test output from the
// DEFAULT topology only. `invariant-only.yml` still ships that chain, still runs the same prompts, and
// is now the only packaged topology that authors generated tests, so the assertions that used to read
// the invariant contract out of the default topology read it here instead. The default topology is
// still asserted, from the other side, to carry no campaign chain and no generated-test manifest, so
// neither half can drift without a failure.
...
function topologyNodes(topologyPath: string): TopologyNode[] {
  return (YAML.parse(readFileSync(topologyPath, "utf8")) as { nodes: TopologyNode[] }).nodes;
}
...
describe("prompt semantic anchors", () => {
  ...
  it("keeps the final invariant campaign backend-neutral on the single recon-fuzzer backend", () => {
    ...
    const topologySource = readFileSync(INVARIANT_ONLY_TOPOLOGY_PATH, "utf8");
    const defaultTopologySource = readFileSync(DEFAULT_TOPOLOGY_PATH, "utf8");
    const invariantNodes = topologyNodes(INVARIANT_ONLY_TOPOLOGY_PATH);
    const defaultNodes = topologyNodes(DEFAULT_TOPOLOGY_PATH);
    const campaignNode = invariantNodes.find((node) => node.id === "stateful-invariant-campaign");
    ...
    // In `invariant-only.yml` the campaign must still reach the review fan-in through its own
    // dependencies; `dynamic-strategy-generator` is not part of that topology, so the default topology
    // carries the other half of the same guard: it declares no invariant chain and nothing depends on
    // the campaign, which is exactly what the control removed.
    expect(invariantNodes.find((node) => node.id === "dedupe-findings")?.depends_on).toContain(
      "stateful-invariant-campaign"
    );
    expect(defaultNodes.filter((node) => node.id.startsWith("stateful-invariant-"))).toEqual([]);
    expect(defaultNodes.flatMap((node) => node.depends_on ?? [])).not.toContain("stateful-invariant-campaign");
    expect(`${topologySource}\n${defaultTopologySource}\n${aggregate}\n${dynamic}`).not.toContain(
      "stateful-invariant-recon-campaign"
    );
    expect(aggregate).toContain("{{ancestor_generated_test_manifests}}");
    // The control's dynamic strategy generator authors no tests, so it reads no generated-test manifest
    // from any node. Asserting the empty derivation keeps a reintroduced manifest reference visible.
  ...
  it("publishes runtime-owned workspace patches for every invariant handoff", () => {
    ...
    // Asserted over every packaged topology that declares these nodes rather than over one topology, so
    // the contract still holds wherever the chain ships: `invariant-only.yml` must declare all five, and
    // the control's default topology declares none. If a chain node is ever added back to the default
    // topology without the patch outputs, this still fails.
    const declaringTopologies = [INVARIANT_ONLY_TOPOLOGY_PATH, DEFAULT_TOPOLOGY_PATH];
    const declaredCounts = new Map<string, number>();

    for (const topologyPath of declaringTopologies) {
      ...
      for (const id of invariantNodeIds) {
        ...
        expect(node.outputs ?? [], `${topologyPath}:${id}`).toEqual(
          expect.arrayContaining([
    ...
    expect([...invariantNodeIds].filter((id) => (declaredCounts.get(id) ?? 0) > 0)).toEqual(invariantNodeIds);
  });
  ...
  it("keeps generated-test manifests on the canonical generated_tests contract", () => {
    ...
    // `invariant-only.yml` is now the only packaged topology whose nodes author generated tests, so the
    // node-side half of the manifest contract is asserted there. The hardcoded `reference-harness-author`
    // check is replaced by an exhaustive derivation over every node whose prompt writes a manifest, so a
    // renamed or newly added authoring node cannot slip past this test.
    const invariantNodes = topologyNodes(INVARIANT_ONLY_TOPOLOGY_PATH);
    const requiredArtifactsById = new Map(
      invariantNodes.map((node) => [node.id, (node.outputs ?? []).map((output) => output.path)])
    );
    const assets = loadBuiltInPromptAssets();
    const promptCorpus = assets.map((asset) => asset.markdown).join("\n");
    const manifestWritingPromptPaths = new Set(
      assets
        .filter((asset) => asset.markdown.includes("{{artifact_dir}}/generated-tests.json"))
    ...
    // The control's dynamic strategy generator no longer plans generated tests, so it carries no
    // manifest key at all; the canonical-key guard below covers every prompt that still writes one.
    expect(dynamic).not.toContain("generated_tests");
    // Pins which prompts author a manifest at all: in the control only the retained invariant chain
    // does, so a strategy or review prompt drifting back into test authoring shows up here.
    expect([...manifestWritingPromptPaths].sort()).toEqual([
      "strategies/invariants/coverage.md",
      "strategies/invariants/implement-properties.md",
      "strategies/invariants/invariant-testing-campaign.md"
    ]);
    expect(`${readFileSync(templatePath, "utf8")}\n${promptCorpus}`).not.toContain("test_files");
    const authoringNodeIds = invariantNodes
      .filter((node) => node.prompt !== undefined && manifestWritingPromptPaths.has(node.prompt))
      .map((node) => node.id);
    expect(authoringNodeIds.length).toBeGreaterThan(0);
    for (const nodeId of authoringNodeIds) {
      expect(requiredArtifactsById.get(nodeId), nodeId).toContain("generated-tests.json");
    }
    // The other half: the control's default topology declares no generated-test output and reaches no
    // prompt that writes a manifest, so a reintroduced authoring node fails here instead of silently
    // producing an unverified manifest.
    const defaultNodes = topologyNodes(DEFAULT_TOPOLOGY_PATH);
    expect(defaultNodes.flatMap((node) => (node.outputs ?? []).map((output) => output.path))).not.toContain(
      "generated-tests.json"
    );
    expect(
      defaultNodes.filter((node) => node.prompt !== undefined && manifestWritingPromptPaths.has(node.prompt))
    ).toEqual([]);
  ...
  // `strategies/admin-config-boundaries.md` is exclusive to the default topology, and the NoFuzz control
  // reframes it from authoring target-native tests to source-only bug search. The target-native and
  // generated-test-companion anchors therefore have no subject left; they are replaced by the control's
  // own contract plus negative guards, so test authoring, workspace writes, or a generated-test manifest
  // drifting back into this prompt still fails here instead of quietly re-arming the removed capability.
  it("keeps admin/config analysis source-backed with no generated-test authoring", () => {
    const admin = prompt("strategies/admin-config-boundaries.md");

    expect(admin).toContain("find bugs associated with documented admin/configuration");
    ...
    expect(admin).not.toContain("{{workspace_path}}");
  });
