...
test("dashboard preview binds confirmation to the comprehensive runtime launch review", async () => {
  ...
  try {
    ...
    const completed = await waitForDashboardCommand(handle.url, String(job.jobId));
...
async function waitForDashboardCommand(dashboardUrl: string, jobId: string): Promise<Record<string, unknown>> {
  const deadline = Date.now() + 60_000;
  for (;;) {
    const response = await fetch(apiUrl(dashboardUrl, `/api/commands/${encodeURIComponent(jobId)}`));
