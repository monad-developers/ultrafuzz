...
- Adds a validated controller-to-worker selected-task handoff so a Modal node worker executes only the attempt its controller materialized, and refuses a handoff that does not reproduce the canonical task identity (#182).
