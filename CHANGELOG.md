...
- Runs created before sealed controls and authenticated workflow-link journals keep their stored artifacts, but lifecycle and live-inspection commands now fail closed and require a new run ID instead of synthesizing historical trust. `ultrafuzz init` preserves every existing agent adapter unless the operator explicitly passes `--force`, and preserved outdated adapters receive manual update guidance (#165, #413, #569).
