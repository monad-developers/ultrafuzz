...
- The Modal pre-model launch retry budget is now charged against consecutive attempts that never reached model work instead of every attempt a model has made in the generation, so a long run is no longer abandoned as `recovery-budget-exhausted` because earlier attempts did real model work and then asked for a durable resume. The outer no-progress recovery bound is unchanged (#267).
