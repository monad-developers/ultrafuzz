...
- Final-review tasks now reconstruct current property implementation coverage from the verified catalog, `implemented-properties@2` handoff, and resolved priority configuration, then derive normalized JSON, findings, and recoverable Markdown through the same deterministic projector used by runtime and CLI; malformed, unrenderable, or non-authoritative current inputs fail closed while explicit historical handoffs retain compatibility (#460, #461).
