...
Repair only consensus harness or reference defects from the dual triage outputs:
...
The `differential-red-triage` logical node is expected to provide two fresh
attempt artifact directories. If fewer than two independent triage attempt
directories are present, do not repair; mark the campaign incomplete in
`gap-review.json` and `differential-report-review.json`.
...
Repair a harness or reference defect only when both fresh triage attempts
independently agree it is a harness or reference defect. If the attempts
disagree, or either attempt classifies the red as production bug, spec mismatch,
or unknown, preserve the red and report the disagreement.
