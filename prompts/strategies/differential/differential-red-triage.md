...
as two independent fresh attempts. Your attempt index is `{{attempt_index}}`.
...
agreement across both fresh `differential-red-triage` attempts before treating a
failure as an owned harness/reference defect. Production bugs, spec mismatches,
and unknowns must remain preserved and unweakened.
