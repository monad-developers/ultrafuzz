...
When searching for optional fuzzing or invariant config signals, keep each
probe separate and simple. Use `grep -rIlE "invariant_|function echidna|StdInvariant|targetContract|/// forge-config" test contracts`
...
When checking optional fuzzing tools, use their direct version flags and record
the exact command/output instead of guessing from an invalid subcommand:
...
Do not mark a tool as absent merely because a guessed subcommand such as
`medusa version` fails.
