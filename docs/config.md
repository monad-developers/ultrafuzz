...
profile remains `CodexAgent` against OpenAI; `OpenRouterAgent` is a separate,
opt-in stock agent. Its model value is an OpenRouter catalogue ID and is
preserved exactly.

`OpenRouterAgent` supports API-key authentication only and requires the canonical
`OPENROUTER_API_KEY` environment variable. For API-key auth, `CodexAgent` requires
`OPENAI_API_KEY`;
assigning `OPENROUTER_API_KEY` to `agents.CodexAgent.api_key_env` fails config
...
read from the canonical `OPENROUTER_API_KEY` on the operator side and
handed to the child process as `OPENROUTER_API_KEY`, which is the variable pi
itself looks up. A different `agents.PiAgent.api_key_env` is rejected during
config validation.
