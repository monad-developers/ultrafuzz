...
```bash
bun "{{repo_path}}/.ultrafuzz/ci/target-e2e-artifacts.ts" final-report --repo "{{repo_path}}" --artifact "{{artifact_path}}" --out "{{output_findings_path}}" --findings "{{artifact_path:signal-analysis}}/findings.json" --run-metadata "{{run_metadata_path}}"
```
