...
/**
 * Publication monitor gated by actual main-branch push work.
 *
 ...
 * checks whether newer producer work has exceeded its publication budget.
 *
...
/** A quiet repository is not an outage; give the first newer push its own publication budget. */
export function assessExpectedBenchmarkPublication(freshness, producerRuns, now) {
  if (!Array.isArray(producerRuns)) throw new Error("producer runs must be a JSON array");
  if (!(now instanceof Date) || !Number.isFinite(now.getTime())) throw new Error("a valid current instant is required");
  const publishedAt = Date.parse(freshness.newest.run_timestamp);
  let firstPending;
  let sawEarlierRun = false;
  for (const run of producerRuns) {
    const createdAt = producerCreatedAt(run, now);
    if (run.event !== "push" || run.headBranch !== "main") continue;
    if (createdAt <= publishedAt) sawEarlierRun = true;
    if (createdAt <= publishedAt || run.headSha === freshness.newest.candidate_commit) continue;
    if (firstPending === undefined || createdAt < Date.parse(firstPending.createdAt)) firstPending = run;
  }
  const waitDays =
    firstPending === undefined ? 0 : (now.getTime() - Date.parse(firstPending.createdAt)) / MILLISECONDS_PER_DAY;
  // The workflow requests 1,000 newest runs. Do not silently accept a truncated
  // window when omitted older work could already be overdue.
  if (producerRuns.length >= 1_000 && !sawEarlierRun && waitDays <= freshness.max_age_days) {
    throw new Error("producer run snapshot is truncated before the last publication; publication delay is unknown");
  }
  return {
    ...freshness,
    ...publicationExpectation(firstPending, waitDays, freshness.max_age_days),
    publication_wait_days: waitDays,
    pending_producer_run_id: firstPending?.databaseId ?? null,
    pending_producer_created_at: firstPending?.createdAt ?? null
  };
}

function publicationExpectation(firstPending, waitDays, budgetDays) {
  if (firstPending === undefined) return { status: "fresh", expectation: "idle" };
  if (waitDays > budgetDays) return { status: "stale", expectation: "overdue" };
  return { status: "fresh", expectation: "within-budget" };
}

function producerCreatedAt(run, now) {
  if (
    run === null ||
    typeof run !== "object" ||
    ...
    throw new Error("producer run snapshot has an invalid entry");
  const createdAt = Date.parse(run.createdAt);
  if (createdAt > now.getTime()) throw new Error("producer run snapshot has a future creation time");
  return createdAt;
}
...
export function describeBenchmarkHistoryFreshness(result) {
  if (result.status === "fresh") {
    return [
      result.expectation === undefined
        ? "## Benchmark history is advancing"
        : "## Benchmark publication is not overdue",
      "",
      ...(result.expectation === "idle" ? ["No newer main-branch push requires a publication.", ""] : []),
      ...(result.expectation === "within-budget"
        ? ["New producer work is still within its publication budget.", ""]
        : []),
      ...newestTable(result),
      ""
    ].join("\n");
  ...
  return [
    ...
    ...(result.expectation === "overdue"
      ? [
          `Main-branch push run ${result.pending_producer_run_id}, created at ${result.pending_producer_created_at},`,
          `has waited ${result.publication_wait_days.toFixed(1)} days without a newer publication, past its ${result.max_age_days}-day publication budget.`
        ]
      : [`past the ${result.max_age_days}-day freshness budget.`]),
    ``,
    ...newestTable(result),
    ``,
...
function main(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === "--history" && args[index + 1] !== undefined) {
      ...
      index += 1;
    } else if (argument === "--producer-runs" && args[index + 1] !== undefined) {
      producerRunsPath = args[index + 1];
      index += 1;
    } else {
      throw usageError();
    }
  ...
  if (
    ...
    maxAgeDaysText === undefined ||
    reportPath === undefined ||
    producerRunsPath === undefined
  )
    throw usageError();
  const now = new Date();
  const freshness = assertBenchmarkHistoryFreshness({
    historyPath,
  ...
  const result = assessExpectedBenchmarkPublication(
    freshness,
    JSON.parse(fs.readFileSync(producerRunsPath, "utf8")),
    now
  );
...
function usageError() {
  return new Error(
    "usage: assert-benchmark-history-freshness.mjs --history <history.json> --max-age-days <days> --producer-runs <runs.json> --report <report.md>"
  );
}
